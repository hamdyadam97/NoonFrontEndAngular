...
export class CartComponent implements OnInit {
  ...
  updateQuantity(selectedQty: number, cartItem: any) {
    ...
    if (this.selectedQuantities[cartItem.product.id] != null) {
      this.cart.totalAmount = this.cart.totalAmount - (this.selectedQuantities[cartItem.product.id] * cartItem.productPrice);
    }
  ...
  createCart(cartItemsWithQuantity: { cartItem: any; quantity: number }[]): void {
    ...
    this._cartService.createCart().subscribe({
      next: (res) => {
        ...
        cartItemsWithQuantity.forEach((itemWithQuantity) => {
          ...
          this._cartService.addtoCart(cartItem.productId, res.cartId, quantity).subscribe({
            next: (tr) => {
              console.log("Item added to cart:", cartItem.productName, "Quantity:", quantity);
            },
            error: (err) => {
  ...
  goToCheckout(): void {
    ...
    const cartItemsWithUpdatedQuantities = this.cart.cartItems.map(cartItem => {
      return {
        cartItem: cartItem,
    ...
    // Call createCart() function passing in the updated quantities
